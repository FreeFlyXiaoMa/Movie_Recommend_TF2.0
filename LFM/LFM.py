...
def model_train_process():
    ...
    :return:
    '''
    train_data = read_.get_train_data('ratings.csv')
...
def give_recom_result(user_vec, item_vec, userid):
    ...
    for itemid in item_vec:
        ...
        record_list = list(record.items())      # record.items()=[(itemid,res)]   在此用list转换格式是Python3的不兼容问题
...
def ana_recom_result(train_data,userid,recom_list):
    ...
    '''
    item_info = read_.get_item_info('movies.csv')
    for data_instance in train_data:
